...
namespace {
    ...
    UniqueBgfxHandle<bgfx::TextureHandle> LoadImage(const tinygltf::Image& image, bool sRGB) {
        ...
        if (rgbaBuffer == nullptr) {
            return UniqueBgfxHandle<bgfx::TextureHandle>();
        }
            ...
            for (const auto& primitiveBuilderPair : primitiveBuilderMap) {
                ...
                if (materialIndex == -1) // No material was referenced. Make up a material for it.
                {
                    ...
                    auto loadTexture = [&](const char* _name,
                        ...
                        UniqueBgfxHandle<bgfx::TextureHandle> textureView = std::move(imageMap[imageKey]);
                        ...
                        UniqueBgfxHandle<bgfx::UniformHandle> samplerState = std::move(samplerMap[texture.Sampler]);
