...
uniform float4 u_eyePosition;
uniform float3x3 u_highlightPositionLightDirectionLightColor;
uniform float4 u_numSpecularMipLevelsAnimationTime;
uniform float4 u_baseColorFactor;
uniform float4 u_metallicRoughnessNormalOcclusion;
uniform float4 u_emissiveAlphaCutoff;
uniform SamplerState u_baseColorTextureSampler : register(s[0]); uniform Texture2D u_baseColorTextureTexture : register(t[0]); static BgfxSampler2D u_baseColorTexture = { u_baseColorTextureSampler, u_baseColorTextureTexture };
uniform SamplerState u_metallicRoughnessTextureSampler : register(s[1]); uniform Texture2D u_metallicRoughnessTextureTexture : register(t[1]); static BgfxSampler2D u_metallicRoughnessTexture = { u_metallicRoughnessTextureSampler, u_metallicRoughnessTextureTexture };
uniform SamplerState u_normalTextureSampler : register(s[2]); uniform Texture2D u_normalTextureTexture : register(t[2]); static BgfxSampler2D u_normalTexture = { u_normalTextureSampler, u_normalTextureTexture };
uniform SamplerState u_occlusionTextureSampler : register(s[3]); uniform Texture2D u_occlusionTextureTexture : register(t[3]); static BgfxSampler2D u_occlusionTexture = { u_occlusionTextureSampler, u_occlusionTextureTexture };
uniform SamplerState u_emissiveTextureSampler : register(s[4]); uniform Texture2D u_emissiveTextureTexture : register(t[4]); static BgfxSampler2D u_emissiveTexture = { u_emissiveTextureSampler, u_emissiveTextureTexture };
uniform SamplerState u_BRDFTextureSampler : register(s[5]); uniform Texture2D u_BRDFTextureTexture : register(t[5]); static BgfxSampler2D u_BRDFTexture = { u_BRDFTextureSampler, u_BRDFTextureTexture };
uniform SamplerState u_specularTextureSampler : register(s[6]); uniform TextureCube u_specularTextureTexture : register(t[6]); static BgfxSamplerCube u_specularTexture = { u_specularTextureSampler, u_specularTextureTexture };
uniform SamplerState u_diffuseTextureSampler : register(s[7]); uniform TextureCube u_diffuseTextureTexture : register(t[7]); static BgfxSamplerCube u_diffuseTexture = { u_diffuseTextureSampler, u_diffuseTextureTexture };
...
{
float lod = perceptualRoughness * u_numSpecularMipLevelsAnimationTime.x;
float3 brdf = bgfxTexture2D(u_BRDFTexture, float2(NdotV, 1.0 - perceptualRoughness)).rgb;
float3 diffuseLight = bgfxTextureCube(u_diffuseTexture, n).rgb;
float3 specularLight = bgfxTextureCubeLod(u_specularTexture, reflection, lod).rgb;
...
void main( float4 gl_FragCoord : SV_POSITION , float3x3 v_TBN : TANGENT , float4 v_color0 : COLOR0 , float3 v_positionWorld : TEXCOORD1 , float2 v_texcoord0 : TEXCOORD0 , out float4 bgfx_FragData0 : SV_TARGET0 )
{
