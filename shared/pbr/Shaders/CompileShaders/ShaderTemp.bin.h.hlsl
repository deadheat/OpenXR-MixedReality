...
// C:\Users\t-ololu\source\repos\bgfx\.build\win32_vs2017\bin\shadercDebug -i ./ --type vertex --debug -O 0 --platform windows -p vs_5_0 -O 3 -f vs_bump_instanced.sc -o ShaderTemp.bin.h
...
Output main( float4 a_normal : NORMAL , float3 a_position : POSITION , float4 a_tangent : TANGENT , float2 a_texcoord0 : TEXCOORD0 , float4 i_data0 : TEXCOORD7 , float4 i_data1 : TEXCOORD6 , float4 i_data2 : TEXCOORD5 , float4 i_data3 : TEXCOORD4) { Output _varying_; _varying_.v_bitangent = float3(0.0, 1.0, 0.0);; _varying_.v_normal = float3(0.0, 0.0, 1.0);; _varying_.v_tangent = float3(1.0, 0.0, 0.0);; _varying_.v_texcoord0 = float2(0.0, 0.0);; _varying_.v_view = float3(0.0, 0.0, 0.0);; _varying_.v_wpos = float3(0.0, 0.0, 0.0);;
{
float4x4 model;
model[0] = i_data0;
model[1] = i_data1;
model[2] = i_data2;
model[3] = i_data3;
float3 wpos = instMul(model, float4(a_position, 1.0) ).xyz;
_varying_.gl_Position = mul(u_viewProj, float4(wpos, 1.0) );
float4 normal = a_normal * 2.0 - 1.0;
float3 wnormal = instMul(model, float4(normal.xyz, 0.0) ).xyz;
float4 tangent = a_tangent * 2.0 - 1.0;
float3 wtangent = instMul(model, float4(tangent.xyz, 0.0) ).xyz;
_varying_.v_normal = wnormal;
_varying_.v_tangent = wtangent;
_varying_.v_bitangent = cross(_varying_.v_normal, _varying_.v_tangent) * tangent.w;
float3x3 tbn = float3x3(_varying_.v_tangent, _varying_.v_bitangent, _varying_.v_normal);
_varying_.v_wpos = wpos;
float3 weyepos = mul(float4(0.0, 0.0, 0.0, 1.0), u_view).xyz;
_varying_.v_view = instMul(weyepos - wpos, tbn);
